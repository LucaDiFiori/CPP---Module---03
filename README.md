...
- **Same Function Signature**:
	- The function in the derived class must have the same name, same parameters, and same return type as the function in the base class.
	- If any of these differ, it's not considered overriding (it could be overloading instead).

- **Virtual Functions**:
	- To enable overriding, the function in the base class must be declared as virtual.
	- This signals to the compiler that you want to allow derived classes to **override** this function
	- If the function in the base class is not marked as virtual, the base class version of the function will always be called, even if the derived class provides its own implementation

- **Dynamic Dispatch**:
	- Function overriding relies on runtime polymorphism. When you use a base class pointer or reference to an object of the derived class, the appropriate function (base or derived) is called depending on the actual type of the object at runtime
...
class Base {
public:
    // Base class function marked as virtual to allow overriding
    virtual void show() {
...
class Derived : public Base {
public:
    // Overriding the Base class function in Derived class
    void show() override {
...
#### Explanation
- **Base Class Version (Base::show())**:
	- The Base class has a function show() that is declared as virtual.
	- This allows the function to be overridden in derived classes.
- **Derived Class Version (Derived::show())**:
	- The Derived class overrides the show() function, providing its own implementation.
	- The override keyword is optional but helps to clarify that the intention is to override a function.
- **Polymorphism**:
	- In the main() function, we use a base class pointer (Base* ptr) to point to both a Base object and a Derived object.
	- When ptr points to a Base object, Base::show() is called.
	- When ptr points to a Derived object, Derived::show() is called because show() is virtual, allowing dynamic dispatch (i.e., the function call is determined at runtime based on the actual type of the object).

## Why Use virtual and Pointers?
- **Without virtual**: If show() were not marked as virtual, the function call would be resolved based on the type of the pointer at compile-time, and the base class version would be called every time, regardless of the actual type of the object the pointer points to.
```C++
#include <iostream>
using namespace std;
...
int main() {
    ...
    ptr = &d;
    ptr->show();  // Calls Base::show() because show() is not virtual
...
- **With virtual**: The compiler generates code to check the actual type of the object (not just the pointer type) at runtime and calls the appropriate version of the function (base or derived).

#### Summary of Function Overriding in C++
- The base class declares a function as virtual to allow it to be overridden.
- The derived class provides its own implementation of the function, overriding the base class's version.
- When a pointer or reference to the base class is used, C++ uses dynamic dispatch to call the correct function (from either the base or derived class), based on the actual type of the object at runtime.
